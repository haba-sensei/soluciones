...
if(isset($_SESSION['consulta'])){
	if($_SESSION['consulta'] > 0){
		...
		$sql="SELECT * FROM compras WHERE mes =(SELECT MAX('$idp') FROM compras) and year = '$year' ";
        $mes_format = $idp;
        switch ($mes_format) {
            case "01":
                $mes_actual = "ENERO";
		...
		$sql="SELECT * FROM compras WHERE mes =(SELECT MAX('$mes') FROM compras) and year = '$year'";
	}
}else{
	$sql="SELECT * FROM compras WHERE mes =(SELECT MAX('$mes') FROM compras) and year = '$year'";
}
...
$sql2="SELECT * FROM compras WHERE  year = '$year'";
...
//numero de compras
        ...
        $sql3="SELECT * FROM compras WHERE  mes = '$idp' and year = '$year'";
        }else{
            $sql3="SELECT * FROM compras WHERE  mes = '$mes' and year = '$year'";
        }
        }else{
            $sql3="SELECT * FROM compras WHERE  mes = '$mes' and year = '$year'";
        }   
        ...
        $sql4="SELECT `compras`.*,`compras`.`monto`,`compras`.`mes`, `compras`.`year` FROM `compras` WHERE `compras`.`monto` =(SELECT MAX(monto)) AND `compras`.`mes` = '$idp' AND `compras`.`year` = '$year'";
        }else{
        $sql4="SELECT `compras`.*,`compras`.`monto`,`compras`.`mes`, `compras`.`year` FROM `compras` WHERE `compras`.`monto` =(SELECT MAX(monto)) AND `compras`.`mes` = '$mes' AND `compras`.`year` = '$year'";
        }
        }else{
        $sql4="SELECT `compras`.*,`compras`.`monto`,`compras`.`mes`, `compras`.`year` FROM `compras` WHERE `compras`.`monto` =(SELECT MAX(monto)) AND `compras`.`mes` = '$mes' AND `compras`.`year` = '$year'";
        }
    ...
    <div class="card animated fadeInUp delay-01s bg-light">
        ...
        <div class="card-body" style="border-left: 1px solid #d6b7b7;">
            <div class="row align-items-center">
                <div style="position: absolute; background: #3164e4; height: 26%; width: 40.3%; left: 0; top: 0;"><span
                ...
                <div class="col col-6" align="center"
                    ...
                    <div class="icon p-0 fs-19 text-primary opacity-50 "><?php echo $year;  ?>
                    </div>
                </div>
                <div class="col col-6">
        ...
        <a href="" data-toggle="modal" data-target="#modalMes">
            <div class="card-body" style="border-left: 1px solid #d6b7b7;">
                <div class="row align-items-center">
                    ...
                    <div class="col col-6" align="center"
                        style="position: relative; left: 54px; top: 8px; transform: translateX(-50%); ">
                        <div class="icon p-0 fs-19 text-primary opacity-50 "><?php 
                        if (empty($mes_actual)){
                                switch ($mes) {
                                case "01":
                                    $mes_actual = "ENERO";
...
<div class="col col-12 col-md-6 col-xl-3">
    <div class="card animated fadeInUp delay-03s bg-light">
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col col-5">
                    <div class="icon p-0 fs-48 text-primary opacity-50 icofont-coins"></div>
                </div>
                <div class="col col-7">
...
<div class="col col-12 col-md-6 col-xl-3">
    <div class="card animated fadeInUp delay-04s bg-light">
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col col-5">
                    <div class="icon p-0 fs-48 text-primary opacity-50 icofont-dollar-minus">
                    </div>
                </div>
                <div class="col col-7">
                    <h6 class="mt-0 mb-1 text-nowrap">Compra del Mes</h6>
